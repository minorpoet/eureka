...
public class EurekaBootStrap implements ServletContextListener {
    ...
    protected void initEurekaServerContext() throws Exception {
        // 第一步，加载 eureka-server.properties 文件的配置
        ...
        // 第三步， 初始化 eureka-server 内部的 eureka-client（用来和其他 eureka-server 节点进行通信）
        if (eurekaClient == null) {
            EurekaInstanceConfig instanceConfig = isCloud(ConfigurationManager.getDeploymentContext())
                    ? new CloudInstanceConfig()
                    // 这里边会加载 eureka-client.properties 文件里边的配置
                    // （和 DefaultEurekaServerConfig 里边加载eureka-server.properties类似)
